...
Replace `integration_prototype_test.go` with an integration test that exercises pidchain end-to-end the way Claude Desktop exercises caller-probe: a caller binary launches a probe binary as a stdio child, the probe binary calls pidchain to fingerprint its launcher, the caller captures the JSON output. The test runs this three times against the same probe binary and asserts the three fingerprints are identical.
...
For stdio, the PID source is `os.Getppid()` — the binary's parent IS the caller, the same way caller-probe identifies Claude Desktop via `os.Getppid()`. The 003f and 003g flag handlers will be added when those docs land; for 003e only `--channel=stdio` is implemented.
...
The probe calls both `Chain` and `Fingerprint` separately rather than using `Chain(...).Fingerprint`. `Fingerprint` is the public function dynarelay-style consumers will actually call; the test exercises it directly so any divergence between `Chain(...).Fingerprint` and `Fingerprint(...)` surfaces as a test failure.

### The integration test

`pidchain_stdio_integration_test.go` builds the probe binary into `t.TempDir()`, runs it three times via `exec.Command` (the same mechanism Claude Desktop uses to launch caller-probe), captures stdout each run, and asserts the three runs produced identical fingerprints.
