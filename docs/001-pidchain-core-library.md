...
Consumers at v1: the relay project (server-side attestation of connecting clients) and another consumer (replaces another consumer's existing `probe/` package). Any future tool needing caller identity can import the same library.
...
- **Zero CGo.** All platform primitives via `golang.org/x/sys/unix` (Darwin) and `golang.org/x/sys/windows` (Windows). The prototype probe's `kernel_darwin.go` uses CGo (`import "C"`); pidchain must not. Translate the same libproc calls to the pure-Go sysctl path.
...
Adapted from `the prototype probe/probe/canonical.go` with three changes: `start_time` is removed so fingerprints survive ancestor restarts, `caller_name` is removed because consumers maintain separate paired-client stores and don't need in-hash scoping, and the version tag is `"PC1\x00"` to scope the format to pidchain.
...
Bound each invocation with a 10-second `context.WithTimeout` (matching the prototype probe's `codesignTimeout`). TCC and sandbox restrictions can hang codesign indefinitely; the timeout is mandatory.
...
Known limitation: the `codesign` CLI is fragile — slow, can hang under TCC, text parsing is brittle. Accepted for v1 because it's the only zero-CGo path and matches the prototype probe's proven approach. Switching to the native `Security.framework` would require CGo; future work, not blocking.
...
- `Identify` on a chain verified against a live `the prototype probe:get_caller_signature` report produces a chain with the same number of entries and the same codesign identities at each position.
...
- PID-reuse defense via `start_time` tuples. pidchain does not record or verify start_time in v1; per the relay project's pairing model, fingerprints must survive ancestor restarts. Consumers that want liveness checks add their own layer.
- the relay project's pairing flow, pairing-code exchange, paired-client store — the relay project's features, not library features.
- Removal of another consumer's existing `probe/` package — separate implementation doc once pidchain ships and another consumer migrates.

## Consumers (informational)

### the relay project

On every inbound connection, the relay project obtains the peer PID from the kernel via the bus transport (`SO_PEERCRED`, `LOCAL_PEERPID`, or `GetNamedPipeClientProcessId`) and calls `pidchain.Identify(peerPID)`. the relay project compares `.Fingerprint` against its paired-clients store. No fingerprint header is sent by the caller; identity is entirely derived from the relay project from kernel state. This is an improvement over the header-based model in the relay project's 005e design doc.

### another consumer

another consumer replaces its `probe/` package with a thin wrapper that calls `pidchain.Identify(os.Getpid())` at MCP server startup and caches the result for the process lifetime. The MCP tool previously served by `probe/` returns fields derived from the cached `Identity`. another consumer's `probe/probe.go`, `probe/canonical.go`, `probe/kernel_darwin.go`, and associated code are deleted in a follow-up doc once pidchain is available.
