...
- **Go floor: 1.24.** pidchain's source uses no feature newer than Go 1.21. Matches helm and QuestScribe (both on 1.24), which lets them adopt pidchain without bumping their own Go versions. The `go` directive in `go.mod` is a hard floor since Go 1.21.
